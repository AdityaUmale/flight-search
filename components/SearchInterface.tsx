    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button
          title="Close"
          onClick={onClose}
          className="absolute top-10 right-28 p-2 rounded-full bg-white border-2 border-gray-300 hover:bg-gray-100 transition-colors"
        >
        ...
        <div className="absolute inset-0 flex flex-col space-y-6 p-8">
          {[...Array(5)].map((_, index) => (
            <div key={index} className="flex space-x-4">
              <Skeleton className="h-24 w-24 rounded" /> {/* Image skeleton */}
              <div className="space-y-2 flex-1">
                <Skeleton className="h-4 w-full" /> {/* Title line */}
                <Skeleton className="h-4 w-3/4" /> {/* Subtitle line */}
                <Skeleton className="h-4 w-1/2" /> {/* Additional info line */}
              </div>
            </div>
          ))}
        </div>
